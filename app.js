...
app.post('/contact', (req, res) => {
    ...
    siteData.contacts.push(contactSubmission);
    console.log('Contact Form Submission:', contactSubmission);
    res.redirect('/contact');
});

app.post('/book-service', (req, res) => {
    const { userName, serviceTitle, bookingDate, request, status = 'pending' } = req.body;
    const booking = {
        userName,
        serviceTitle,
        bookingDate,
...
app.get('/login', (req, res) => {
    res.render('login', { ...siteData, page: 'login' });
});
...
app.post('/admin/add-service', (req, res) => {
    ...
    siteData.services.push({ title, img, description });
    console.log('Service added:', { title, img, description });
    res.redirect('/admin');
});

app.post('/admin/remove-service', (req, res) => {
    const { index } = req.body;
    if (index >= 0 && index < siteData.services.length) {
        const removedService = siteData.services.splice(index, 1);
        console.log('Service removed:', removedService);
    }
    res.redirect('/admin');
});

app.post('/admin/edit-service/:index', (req, res) => {
    const { index } = req.params;
    const { title, img, description } = req.body;
    if (index >= 0 && index < siteData.services.length) {
        siteData.services[index] = { title, img, description };
        console.log('Service edited:', { title, img, description });
    }
    res.redirect('/admin');
});

app.post('/admin/remove-contact', (req, res) => {
    const { index } = req.body;
    if (index >= 0 && index < siteData.contacts.length) {
        const removedContact = siteData.contacts.splice(index, 1);
        console.log('Contact removed:', removedContact);
    }
    res.redirect('/admin');
});

app.post('/admin/remove-booking', (req, res) => {
    const { index } = req.body;
    if (index >= 0 && index < siteData.bookings.length) {
        const removedBooking = siteData.bookings.splice(index, 1);
        console.log('Booking removed:', removedBooking);
    }
    res.redirect('/admin');
});
